...
export async function GET(req: Request, { params }: { params: Promise<{ itemClassId: string }> }) {
  try {
    assertSpecbooksApiKey(req);

    const { itemClassId } = await params;
    if (!itemClassId) {
      return NextResponse.json({ error: "itemClassId is required" }, { status: 400 });
    }
